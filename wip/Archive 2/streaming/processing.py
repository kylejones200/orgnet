...
def process_email_stream(
    ...
    if processor.classify:
        try:
            from classification.unified import classify_emails, UnifiedClassifier
            classifier = UnifiedClassifier()
            classifications = classify_emails(result_df, classifier)
            from classification.unified import add_classifications_to_dataframe
    ...
    if processor.detect_anomalies:
        try:
            from anomaly_detection.temporal import detect_volume_spikes
...
def real_time_classify(
    ...
    try:
        from classification.unified import UnifiedClassifier, classify_emails
        from classification.unified import add_classifications_to_dataframe
